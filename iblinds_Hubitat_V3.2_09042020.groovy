...
def updated() {
    // When device is updated call intialize and  and set daily schedule for battery refresh
    log.debug "Updated..."
    initialize()
...
def getBattery() {
    ...
    def cmd = []
    cmd << zwave.batteryV1.batteryGet().format()
    sendHubCommand(new hubitat.device.HubMultiAction((cmd), hubitat.device.Protocol.ZWAVE))
}
